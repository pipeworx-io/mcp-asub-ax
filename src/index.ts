...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Statistics and Research Åland');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'subjects',
    description: 'Browse the ÅSUB (Statistics Åland) PxWeb subject tree. Pass a sub-path like "Statistik" or "Statistik/BE" to list folders (type "l") and tables (type "t", id ends in ".px"); omit path to list the top-level databases (Statistik, Utredning).',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'table_meta',
    description: 'Fetch dimension definitions and valid values for an ÅSUB PxWeb table (.px file). Path must be the full table path ending in ".px" (e.g. "Statistik/BE/Befolkningsrörelsen/BE006.px"). Returns dimension codes, labels, and the value lists needed to build a query_table body.',
    inputSchema: {
      type: 'object',
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'query_table': {
      ...
      const res = await pwFetch(`${BASE}/${encodePath(path)}`, {
        method: 'POST',
